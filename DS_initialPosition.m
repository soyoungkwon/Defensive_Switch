...
% mode timing
switchT = repmat([6 5 7.5 6.5 8 9 7 8 7 9 7.5]+20, [1 4]);
...
moveXYarray = []; %dist4ok = [];
dotMatrix = []; tcaught = []; caughtA = zeros(1,2);
...
for i = 1:nframes
    ...
    if (i>0)
        ...
        if mod(i, switchframe(sw))==1 %|| sum(abs(xy14-xy24))==0 % switch mode, either time has reached or prey=predator 
            AEmode = -AEmode;
            sw = sw+1;
